defmodule TableFormatterTest do
  ...
  test "correct format string returned" do
    formatted_string = TF.format_for([9,10,11])
    # IO.inspect(String.graphemes(formatted_string))
    # IO.inspect(String.graphemes("~-9s / ~-10s / ~-11s~n"))
    # IO.inspect(String.equivalent?(formatted_string, "~-9s / ~-10s / ~-11s~n") )
    assert formatted_string == "~-9s / ~-10s / ~-11s~n"
  end
